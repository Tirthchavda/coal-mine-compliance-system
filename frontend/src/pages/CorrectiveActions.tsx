...
export const CorrectiveActions: React.FC = () => {
  ...
  const fetchActions = async () => {
    try {
      const res = await api.get('/actions', { timeout: 8000 });
      if (res.data?.success && res.data?.data && res.data.data.length > 0) {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between text-xs text-slate-500 px-1">
        <span>
          Showing <strong className="text-slate-800">{filteredActions.length}</strong> of <strong className="text-slate-800">{actions.length}</strong> CAPA remediation tasks
        </span>
        {(search || mineId || statusFilter || overdueOnly) && (
